...
module Jekyll
  module TOCGenerator
    TOGGLE_HTML = '<div id="toc-title"><span>%1</span>%2</div>'
    TOC_CONTAINER_HTML = '<div id="toc-container" class="no-print well well-sm"><div class="toc">%1<ul class="toc-outermost">%2</ul></div></div>'
    ...
    def toc_generate(html)
      ...
      return html if @context.environments.first['page']['noToc']
      ...
      doc.css(toc_top_tag).each do |tag|
        ...
        sects.each do |sect|
          ...
          level_html += create_level_html(anchor_id,
                                          ...
                                          "#{item_number}.#{inner_section}",
                                          sect.text,
                                          '')
        end

        level_html = "<ul>#{level_html}</ul>" if level_html.length.positive?

        anchor_id = "#{anchor_prefix}#{toc_level}-#{toc_section}"
      ...
      return html unless toc_html.length.positive?
